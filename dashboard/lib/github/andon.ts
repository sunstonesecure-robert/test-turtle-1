...
export function unpublishedAndonFromBody(body: string): { runId: string | null; planRef: string | null } {
  // The run id comes from gh-aw's OWN footer, never from the agent's prose: a plan
  // may cite an earlier build or evidence run, and the first run link in the body
  // would point the publisher at the wrong run (Codex on PR #209). Two anchors gh-aw
  // writes, in order of preference: the structured marker comment, then the
  // "Generated by [...](…/actions/runs/N)" footer line — the LAST such line, since
  // the footer is appended.
  const marker = /<!--\s*gh-aw-agentic-workflow:[^>]*\bid:\s*(\d+)/.exec(body);
  const footers = [...body.matchAll(/^>\s*Generated by \[[^\]]*\]\([^)]*\/actions\/runs\/(\d+)(?!\d)/gm)];
  const runId = marker?.[1] ?? footers.at(-1)?.[1] ?? null;
  const ref = /\*\*Plan ref:\*\*\s*`(plan\/[a-z0-9][a-z0-9-]*\/v\d+)`/.exec(body);
  return { runId, planRef: ref?.[1] ?? null };
}

/**
 * Only a LIVE break can be "unpublished". An ordinary issue, a pull request, or a
 * resolved/superseded break with no header is not a review waiting on the
 * publisher — rendering the recovery page for it would offer a write-capable
 * action on the wrong thing (Codex on PR #209). Pure, so it is tested directly.
 */
export function isRecoverableHeaderless(labels: string[], isPullRequest: boolean): boolean {
  return !isPullRequest && isLiveAndon(labels);
}
...
/**
 * The RESOLVED break that names a plan ref — the freeze's completion marker (GHI #212,
 * review of PR #214). `freezeApprovedPlan` adds `andon:resolved` only AFTER the tag, the
 * flags and the work-item reconciliation, so a frozen tag whose break is not resolved is
 * a freeze that has not finished: its items may still say what the previous version
 * said. Closed issues only; the live lookup above is a different question.
 */
export async function findResolvedAndonByPlanRef(gh: Octokit, repo: RepoRef, planRef: string): Promise<number | null> {
